...
opts=`getopt -o c:d:f:ghk:p:qt:P: --long conf-disable,int-port-loop:,log-dir:,json-logs-enable,pkt-log-len:,use-pcie-veth,dod-test-mode,arch:,time-disable,no-port-monitor,cli-port: -- "$@"`
...
while true; do
    case "$1" in
      ...
      -t) TCP_PORT_BASE=$2; shift 2;;
      -P|--cli-port) CLI_PORT=$2; shift 2;;

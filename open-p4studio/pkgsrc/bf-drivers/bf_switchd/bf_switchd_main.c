...
static void bf_switchd_parse_options(bf_switchd_context_t *ctx,
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case OPT_SHELL_NO_WAIT:
        ...
        break;
      case OPT_PD_API_PORT:
        ctx->pd_api_port = atoi(optarg);
        break;
      case OPT_SWITCHD_LISTEN_PORT:
        ctx->switchd_listen_port = atoi(optarg);
        break;
      case 'h':
      case '?':
        printf("bf_switchd \n");
        ...
        printf(" --pd-api-port=Port number for PD API server\n");
        printf(" --switchd-listen-port=Port number that switchd listens\n");
  ...
  if (ctx->pd_api_port == 0) {
    ctx->pd_api_port = 9090;
  }
  if (ctx->switchd_listen_port == 0) {
    ctx->switchd_listen_port = 9999;
  }

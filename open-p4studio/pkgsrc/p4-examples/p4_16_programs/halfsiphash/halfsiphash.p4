...
header sip_inout_h {
	...
	bit<32> m_4; // Note: For now this is hardcoded, but later we may want to define it using macro
}
...
parser SwitchIngressParser(
	...
	state parse_ipv4 {
		...
		transition select(hdr.ipv4.protocol) {
			VALIDATION_PROTO : parse_sip;
			SIP_PROTO : parse_sip_and_meta;
			SIP_2_PROTO : parse_sip_and_meta;
			default : accept;
		}
	}
}
...
control SwitchIngress(
	...
	action start_for_svf() {
		ig_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		hdr.sip.m_0 = hdr.sip.m_4;
		hdr.sip.m_1 = 0;
		hdr.sip.m_2 = 0;
		hdr.sip.m_3 = 0;
		MSG_VAR_IG = hdr.sip.m_0;
	}

	action start_m_0_compression() {
		ig_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_IG = hdr.sip.m_0;
	}

	action start_m_1_compression() {
		ig_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_IG = hdr.sip.m_1;
	}

	action start_m_2_compression() {
		ig_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_IG = hdr.sip.m_2;
	}

	action start_m_3_compression() {
		ig_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_IG = hdr.sip.m_3;
	}
	...
	apply {
		...
		if(!is_sip){
			...
			if(hdr.ipv4.protocol == VALIDATION_PROTO){
				start_first_pass_revf();
				hdr.ipv4.protocol= SIP_PROTO;
			}
			else if(hdr.ipv4.protocol == SIP_PROTO && hdr.sip_meta.revf_result != 0){
				start_first_pass_svf();
				hdr.ipv4.protocol = SIP_2_PROTO;
			}
			else
...
control SwitchEgress(
	...
	action final_round_revf(){
		#define eg_writeout_m(i) hdr.sip.m_##i = 0;
		__LOOP(NUM_WORDS,eg_writeout_m)
		@in_hash { hdr.sip_meta.revf_result = hdr.sip_meta.v_1 ^ hdr.sip_meta.v_3; }
		hdr.sip.m_0 = hdr.sip.m_4;
	}

	action final_round_svf(){
		@in_hash { hdr.sip.m_1 = hdr.sip_meta.v_1 ^ hdr.sip_meta.v_3; }
		hdr.sip.m_0 = hdr.sip_meta.revf_result;
		hdr.sip_meta.setInvalid();
	}
	...
	*/
	...
	action start_m_0_compression() {
		...
		MSG_VAR_EG = hdr.sip.m_0;
	}

	action start_m_1_compression() {
		eg_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_EG = hdr.sip.m_1;
	}

	action start_m_2_compression() {
		eg_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_EG = hdr.sip.m_2;
	}

	action start_m_3_compression() {
		eg_md.sip_tmp.round_type = ROUND_TYPE_COMPRESSION;
		MSG_VAR_EG = hdr.sip.m_3;
	}
	...
	apply {
		...
		if(hdr.sip_meta.curr_round < (NUM_WORDS*2+2)){
			...
			if (hdr.ipv4.protocol == SIP_PROTO) {
				hdr.sip_meta.curr_round = 0;
				final_round_revf();
			}
			else
